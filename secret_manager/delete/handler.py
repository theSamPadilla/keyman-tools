"""Handler for the 'delete' subcommand of secret-manager command"""

import sys

import secret_manager.utilities as util
import secret_manager.delete.utilities as de_util
import secret_manager.delete.delete_secrets as del_executer
...
def handler(subcommand_flags: list, project_id: str):
    ...
    if not secret_name and not pattern:
        print (f"[{red}ERROR{end}] No delete pattern was passed.",
               f"\n\tRun {print_usage_string_for_command_and_flag('secret-manager', '--help')} to see more.")
        sys.exit(1)
    ...
    print(f"[{green}SUCCESS{end}] Succesfully deleted {deleted_secrets} secrets.")
